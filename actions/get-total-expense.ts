...
export const getTotalExpense = async (accountId: string): Promise<{ month: string, total: number }[]> => {
  ...
  const calculateTotalExpense = (transactions: any[]) => {
    return transactions.reduce((result: number, item: any) => {
      if (item.category.activity === 'Expense') {
        result += item.amount;
      }
      return result;
    }, 0);
  ...
  const currentMonthExpense = calculateTotalExpense(currentMonthTransactions || []);
  const previousMonthExpense = calculateTotalExpense(previousMonthTransactions || []);

  const resultArray = [
    { month: startOfPreviousMonth.toLocaleString('default', { month: 'long' }), total: previousMonthExpense },
    { month: startOfCurrentMonth.toLocaleString('default', { month: 'long' }), total: currentMonthExpense }
  ];
