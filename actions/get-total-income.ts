...
export const getTotalIncome = async (accountId: string): Promise<{ month: string, total: number }[]> => {
  ...
  const currentMonthIncome = calculateTotalIncome(currentMonthTransactions || []);
  const previousMonthIncome = calculateTotalIncome(previousMonthTransactions || []);

  const resultArray = [
    { month: startOfPreviousMonth.toLocaleString('default', { month: 'long' }), total: previousMonthIncome },
    { month: startOfCurrentMonth.toLocaleString('default', { month: 'long' }), total: currentMonthIncome }
  ];
